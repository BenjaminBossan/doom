...
(defun my-convert-md-to-rst-py (beg end)
  "some docstring."
  (interactive "r")
  ;(uncomment-region beg end)
  (setq reg (buffer-substring beg end))
  (setq reg (s-replace-regexp "^# " "" reg))
  (setq reg (s-replace-regexp "^%% \\[markdown\\]" "%%\n          " reg))
  (message "%s" reg)
  (setq reg (s-replace-regexp "^ " "" reg))
  (setq reg (s-replace-regexp "`" "XXXXX" reg))
  (setq reg (s-replace-regexp "\"" "YYYYY" reg))
  (setq cmd (concat "pandoc -f markdown -t rst <<< \"" reg "\""))
  (setq rst (shell-command-to-string cmd))
  (setq commented-rst (s-replace-regexp "^" "# " rst))
  (setq commented-rst (s-replace-regexp "^# \n" "\n" commented-rst))
  (setq commented-rst (s-replace-regexp "^# $" "\n" commented-rst))
  (setq commented-rst (s-replace-regexp "^# %%\n" "# %%" commented-rst))
  (setq commented-rst (s-replace-regexp "XXXXX" "``" commented-rst))
  (setq commented-rst (s-replace-regexp "YYYYY" "\"" commented-rst))
